...
export class MyAdsAndSettingUtils {
    static async addMyAdsToList(listItem, androidID) {
        if (isEmpty(listItem))
            return false;
    ...
    static async getPreferAds(androidID) {
    ...
    static async _getPreferAdsFromList(apps, androidID) {
        if (isEmpty(apps))
            return undefined;
    ...
    static async canShowMyAds(index, packageName, androidID) {
        if (packageName === androidID)
            return false;
        if (!isIOS()) {
            if (await RNAppUtils.ANDROID_isAppInstalled(packageName))
                return false;
        }
