...
export class AddAdsToListUtils {
     ...
     */
    static async addAdsObjToListDetail(listData, isLargeAds = true, typeAds = NativeAdsView.TYPE_DETAIL_VOCA) {
        if (await RNCommonUtils.isVIPUser()) {
            return;
        }
        if (isEmpty(listData))
    ...
    static _addAdsFollowSpace(listData, currentCountAds, typeAds, countFromDeQui) {
        if (countFromDeQui > 10) {
            sendError("addAdsFollowSpace can Loop Forever ============== ");
            return currentCountAds;
        }
        let space = 0;
        let listIndexStartToEnd = []; // Vị trí có thể thêm Ads. Tính từ trên xuống
        let listIndexEndToStart = []; // Vị trí có thể thêm Ads. Tính từ dưới lên
        let listIndexStartToEndPref = []; // Vị trí có thể thêm Ads. Tính từ dưới lên. Pref Ads
        let listIndexEndToStartPref = []; // Vị trí có thể thêm Ads. Tính từ dưới lên. Pref Ads
        ...
        if (listIndexMatchPref.length == 0)
            return currentCountAds;
        // Add Prefer Ads
    ...
    static _addAdsToBottom(listData, currentCountAds, typeAds) {
        let space = 0;
        for (let i = listData.length - 1; i >= 0; i--) {
            let item = listData[i];
    ...
    static _addAdsAfterResultButton(listData, currentCountAds, typeAds) {
    ...
    static _addAdsCenterContentAndExercise(listData, currentCountAds, typeAds) {
        let space = 0;
        for (let i = 0; i < listData.length; i++) {
            let item = listData[i];
...
function _caculatorSpace(item) {
    ...
    if (item.lesson && item.lesson.practice)
        return 2;
    let type = item.type;
...
function getSpaceOfMultichoice(obj) {
    if (obj == null)
        return 0;
    let result = 0;
    if (obj.img)
...
function getSpaceOfVoca(vocaObj) {
    if (vocaObj == null)
        return 0;
    let result = 2;
    if (vocaObj.sample)
...
function isPreferHtmlDai(listData, index) {
    index--;
    let space = 0;
    while (index >= 0) {
...
function isPartOfText(item) {
    let type = item.type;
    if (type == "html" || type == "text_html" || type == "paragraph" || type == "text" || type == "writing")
        return true;
    return false;
}
function canNotAddAdsCenter(itemBefore, itemAfter) {
